...
>>>>>>> origin/master
    ...
    function verifica_perguntas(){
        contador_perguntas = $('.ml-count').html(); //isto na página de perguntas
        if (contador_perguntas >= 1){
            console.log("Tem perguntas para responder!");
            if (location.host != 'questions.mercadolivre.com.br') location.href='https://questions.mercadolivre.com.br/seller';
        }
    }
    function loop_perguntas(){
        setTimeout(function(){
            console.log("verificou perguntas");
            verifica_perguntas();
            loop_perguntas();
        },30000);
    }
    if (mf_perguntas == 'checked') loop_perguntas();
    // VERIFICA PERGUNTAS PENDENTES //
